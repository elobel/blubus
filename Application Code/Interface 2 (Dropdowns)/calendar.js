...
function openCollege(nm, collegeName, college) {
        ...
        //update the label on the dropdown menu to show the selected college
        dropDown = document.getElementsByClassName("dropbtn1");
        for(i = 0; i < dropDown.length; i++) {
            dropDown[i].innerHTML = college;
        }
    
        //reset the label on the day dropdown menu
        dropDown = document.getElementsByClassName("dropbtn2");
        for(i = 0; i < dropDown.length; i++) {
            dropDown[i].innerHTML = "[Select a Day]";
        }
...
//source: https://www.w3schools.com/howto/howto_js_tabs.asp
function openSchedule(dy, day) {
        // Declare all variables
        var i, tabContent, tablinks;
        var dropDown, location, dayLeaving;
        var college;
        var letter;
    
    
        
        var dySchedule; //the day schedule to be opened
        dySchedule = document.getElementsByClassName("dropbtn1"); //dropbtn1 should have the                                                                 //current college selected
        for(i = 0; i < dySchedule.length; i++) {
            college = dySchedule[i].innerHTML;
            letter = dySchedule[i].innerHTML.substr(0,1); //get the first letter of the college
                                                              //selected
        }
    
    
        //combine dySchedule with day to get the complete id to be opened
        dySchedule = letter + day;
    
    
        //check that a college has been selected
        if(college == '[Select a Location]') { //a college has not yet been selected
            alert("Whoops! Please select a location before selecting a day.");
        }
        ...
        //update the button to show the day chosen
        dropDown = document.getElementsByClassName("dropbtn2");
        for(i = 0; i < dropDown.length; i++) {
            dropDown[i].innerHTML = day;
        }
    
        //update the sentence preceeding the schedule
        location = document.getElementsByClassName("departLocation");
        dayLeaving = document.getElementsByClassName("departDay");
        for(i = 0; i < location.length; i++) {
            location[i].innerHTML = college;
            dayLeaving[i].innerHTML = day;
        }
    }
...
function dropDown(menuChosen) { //reveal the dropdown menu items
    document.getElementById(menuChosen).classList.toggle("show" + menuChosen);  
}



//source: https://www.w3schools.com/howto/howto_js_dropdown.asp
window.onclick = function(event) { //close the drop down menu if user clicks outside of it
  ...
  if ((!event.target.matches(".dropbtn1"))) {
    ...
    for (i = 0; i < dropdowns.length; i++) {
      ...
      if (openDropdown.classList.contains('showdropbtn1')) {
        openDropdown.classList.remove('showdropbtn1');
      }
    }
  }
  if ((!event.target.matches(".dropbtn2"))) {
    ...
    for (i = 0; i < dropdowns.length; i++) {
      openDropdown = dropdowns[i];
      if (openDropdown.classList.contains('showdropbtn2')) {
        openDropdown.classList.remove('showdropbtn2');
      }
    }
  }
...
function setPresetAlarm() {
        ...
        if(presetTime != undefined) {
            alert("A preset has been set for " + presetTime);
            findTimeForAlarm();
		}
    }
...
function saveTime(){ //PUT THE ALARM IN MY ALARMS
	...
	if(custom == 1){
		...
		custom = 0;
		//test.innerText = "called saveTime with custom";
		//test2.innerText = whichCollege + whichWeekday + whatTime;
...
function setToHappen(userTime){ //make the alarm go off
	var weekday = document.getElementById("weekday");
